/**
 * ETLWorker graceful-shutdown tests.
 *
...
function makeWorker(opts: ETLWorkerOptions = {}): WorkerFixture {
  const mockRunETL = jest.fn<() => Promise<void>>().mockResolvedValue(undefined)
...
describe('ETLWorker', () => {
  ...
  describe('start()', () => {
    ...
    it('passes an AbortSignal to the ETL service', async () => {
      const { worker, mockRunETL } = makeWorker()
      worker.start()
      await flushMicrotasks()

      expect(mockRunETL).toHaveBeenCalledWith(expect.any(AbortSignal))
      await worker.stop()
    })
  ...
  describe('stop()', () => {
    ...
    it('clears the interval so no further runs are scheduled', async () => {
      ...
      await flushMicrotasks() // let initial run settle
      await worker.stop()

      jest.advanceTimersByTime(120_000)
      await flushMicrotasks()

      expect(mockRunETL).toHaveBeenCalledTimes(1) // only the initial run
    })
    ...
    it('proceeds after drainTimeoutMs if the run does not finish in time', async () => {
      ...
      mockRunETL.mockImplementationOnce(() => new Promise<void>(() => {})) // never resolves
  ...
  describe('runETL()', () => {
    ...
    it('triggers and awaits a run when the worker is running', async () => {
      const { worker, mockRunETL } = makeWorker()
      worker.start()
      await flushMicrotasks() // settle initial run
    ...
    it('skips if a run is already active', async () => {
      ...
      worker.start()
      await worker.runETL() // should be skipped – run still in progress
      expect(mockRunETL).toHaveBeenCalledTimes(1)
  ...
  describe('AbortSignal', () => {
    it('aborts the in-flight signal when stop() is called', async () => {
      ...
      mockRunETL.mockImplementationOnce((signal: unknown) => {
        ...
        return new Promise<void>(() => {}) // never resolves naturally
      })
